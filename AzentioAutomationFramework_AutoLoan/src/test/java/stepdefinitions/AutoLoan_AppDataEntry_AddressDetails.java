...
public class AutoLoan_AppDataEntry_AddressDetails {
	...
	JSPaths AddressDetails_js = new JSPaths(excelPath, "addressdetails_JSPath", "AddressDetailsFieldName", "JSPaths");
	...
	public void user_6047_Click_the_module_name_dropdown_in_ULS_application() throws Throwable {
		...
		for (int i = 0; i <= 100; i++) {
			...
			try {  moduleName = javascriptHelper
					.executeScript("return " + iJarah_CommonElements.getElement("module_dropdown") + ".textContent")
					.toString();
		...
		for (int i = 0; i <= 100; i++) {
			try {
				javascriptHelper.executeScriptWithWebElement(iJarah_CommonElements.getElement("module_dropdown")).click();
				break;
			} catch (Exception e) {
				if (i == 100) {
					Assert.fail(e.getMessage());
	...
	public void user_6047_click_the_entitle_button_under_inbox() throws Throwable {
		...
		for (int i = 0; i <= 150; i++) {
			try {
				//javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("inboxEntitleBtn")).click();
	...
	public void user_6047_get_the_test_data_for_test_case_id_at_au_ad01() {
		executionData = AutoLoanExecutionExcel.getTestdata("AT_AU_AD_01");
		testData =  AddressDetails_TestData.getTestdata(executionData.get("dataSet_ID"));
	...
	public void user_6047_get_the_test_data_for_test_case_id_at_au_ad09() {
		executionData = AutoLoanExecutionExcel.getTestdata("AT_AU_AD_09");
		testData =  AddressDetails_TestData.getTestdata(executionData.get("dataSet_ID"));
...
//					javascriptHelper.scrollIntoView(addButton);
	...
	public void user_6047_click_on_customer_financial_tab() throws InterruptedException {
		Thread.sleep(700);
		String lengthOfTheSegmentButton = "";
		boolean isClicked = false;
		for (int i = 0; i <= 300; i++) {
			...
			try {
				for (int j = 0; j <= 200; j++) {
					segmentLabelName = javascriptHelper.executeScript(
							...
							.toString();
					System.out.println("Segment Label Name " + segmentLabelName);
					if (segmentLabelName.trim().equalsIgnoreCase("Customer Financials")) {
						System.out.println("Condition true");
						javascriptHelper.executeScriptWithWebElement(
								"document.querySelector('ion-segment').querySelectorAll('ion-segment-button')[" + i
										+ "]")
								.click();
						System.out.println("document.querySelector('ion-segment').querySelectorAll('ion-segment-button')[" + i
										+ "]");
						indexOfListView=i;
						isClicked = true;
						break;

					} else if (!(segmentLabelName.equalsIgnoreCase("Customer Financials"))) {
						isClicked = false;
						break;
					}
				}
	...
	public void user_6047_clict_the_add_button_under_the_address_detail() throws Throwable {
		Thread.sleep(700);
...
Thread.sleep(500);
	...
	public void user_6047_verify_neighbourhood_should_be_mandatory_editable_text_box() {
		...
		String neighbourhoodEdit= javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("neighbourhood_districtName")).getAttribute("ng-reflect-is-disabled");
		for (int i = 0; i <2000; i++) {
            try {
		...
		String NeighbourhoodText = javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("neighbourhood_districtName")).getAttribute("ng-reflect-placeholder");
		for (int i = 0; i <2000; i++) {
            try {
	...
	public void user_6047_verify_city_should_be_mandatory_editable_text_box() {
		...
		String City_Text= javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("City")).getAttribute("type");
		for (int i = 0; i <2000; i++) {
            try {
	...
	public void user_6047_clict_the_edit_button_under_the_address_detail() throws Throwable {
		Thread.sleep(2000);
		String length =null;
		for (int i = 0; i <=150; i++) {
			try {
...
public void user_6047_verify_the_edit_botton_in_address_details_list_view() throws Throwable {
	Thread.sleep(700);
	String length =null;
	for (int i = 0; i <500; i++) {
		try {
...
@Then("User_6047 To search the record in list view of address detail")
public void user_6047_to_search_the_record_in_list_view_of_address_detail() throws Throwable {
	Thread.sleep(700);
	String length =null;
	for (int i = 0; i <500; i++) {
		try {
...
public void user_6047_to_search_the_record_in_list_view_of_address_detail_for_status() throws Throwable {
	for (int i = 0; i <= 1000; i++) {
		try {
			//String path ="document.querySelector('span[class=\"p-input-icon-left ng-star-inserted\"] i')";
			String path ="document.querySelector('span[class=\"p-input-icon-left ng-star-inserted\"] input')";
			WebElement Button = javascriptHelper.executeScriptWithWebElement(path);
			//javascriptHelper.scrollIntoView(javascriptHelper.executeScriptWithWebElement(path));
			javascriptHelper
			.executeScriptWithWebElement(
...
public void user_6047_verify_the_successfully_saved_message_popup() {
	for (int i = 0; i <2000; i++) {
        try {
        	String text = javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("SucessPopUP")).getText();
        	System.out.println(text);
...
public void user_6047_change_the_address_details_status_to_active_to_inactive_or_inactive_to_active() throws Throwable {
	for (int i = 0; i <= 300; i++) {
		try {
			javascriptHelper.scrollIntoView(javascriptHelper
					.executeScriptWithWebElement(AddressDetails_js.getElement("Status")));
			javascriptHelper
					.executeScriptWithWebElement(AddressDetails_js.getElement("Status_toggle"))
					.click();
			break;
		} catch (Exception e) {
			if (i == 300) {
	...
	for (int i = 0; i <= 100; i++) {
		try {
			recordStatus = javascriptHelper
					.executeScript(
							"return " + AddressDetails_js.getElement("Status_current"))
					.toString();
		} catch (Exception e) {
...
public void user_6047_to_click_the_back_button_in_customer_identificatoin( ) {
	for (int i = 0; i <= 1000; i++) {
		try {
			javascriptHelper.JSEClick(
					javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("BackButton")));
...
@Given("User_6047 click the back button in listview screen")
public void user_6047_click_the_back_button_in_listview_screen() {
	for (int i = 0; i <= 1000; i++) {
		try {
			javascriptHelper.JSEClick(javascriptHelper.executeScriptWithWebElement(AddressDetails_js.getElement("BackButton")));
