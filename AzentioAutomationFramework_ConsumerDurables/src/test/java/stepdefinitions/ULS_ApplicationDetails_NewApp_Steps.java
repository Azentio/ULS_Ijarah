...
public class ULS_ApplicationDetails_NewApp_Steps extends BaseClass {
	...
	String excelFilePathForJS = configFileReader.getJSPathFilePath();
	String applicationDetailsExcelDataPath = configFileReader.getTestDataFilePath();
	JSPaths commonJSPaths = new JSPaths(excelFilePathForJS, "CommonElements", "CommonFieldName", "JSPath");
	...
	ExcelData consumerDurablesExcelTestData = new ExcelData(configFileReader.getTestDataFilePath(),
			"ApplicationDetails_DataEntry", "Dataset ID");
	...
	public void user_076_get_the_test_data_for_test_case_id_at_cd_ad_appdetails_01() throws Throwable {
		consumerDurablesExecutionData = consumerDurablesExecutionSheet.getTestdata("AT_CD_AD_APPDETAILS_01");
		System.out.println("Data set ID " + consumerDurablesExecutionData.get("dataSet_ID"));
		newApplicationTestData = consumerDurablesExcelTestData
	...
	public void user_076_get_the_test_data_for_test_case_id_at_cd_ad_appdetails_02() throws Throwable {
		consumerDurablesExecutionData = consumerDurablesExecutionSheet.getTestdata("AT_CD_AD_APPDETAILS_02");
		System.out.println("Data set ID " + consumerDurablesExecutionData.get("dataSet_ID"));
		newApplicationTestData = consumerDurablesExcelTestData
				.getTestdata(consumerDurablesExecutionData.get("dataSet_ID"));
	...
	public void user_076_clear_the_finance_amount_field_input_in_application_details_screen() throws Throwable {
		for (int i = 0; i <= 2; i++) {
			try {
				javascriptHelper.executeScriptWithWebElement(
	...
	public void user_076_clear_the_declared_net_monthly_income_field_input_in_application_details_screen()
			throws Throwable {
		for (int i = 0; i <= 2; i++) {
			try {
				javascriptHelper
	...
	public void user_076_clear_the_declared_current_obligation_field_input_in_application_details_screen()
			throws Throwable {
		for (int i = 0; i <= 2; i++) {
			try {
				javascriptHelper.executeScriptWithWebElement(
	...
	public void user_076_clear_the_topup_application_number_field_input_in_application_details_screen()
			throws Throwable {
		for (int i = 0; i <= 2; i++) {
			try {
				javascriptHelper.executeScriptWithWebElement(
						applicationDetailsElements.getElement("topup_application_number_input")).click();
				javascriptHelper
						.executeScriptWithWebElement(
	...
	public void user_076_clear_the_reference_code_number_field_input_in_application_details_screen() throws Throwable {
		for (int i = 0; i <= 2; i++) {
			try {
				javascriptHelper
	...
	public void user_076_verify_consumer_durables_record_should_get_submitted_from_data_entry_stage_and_moved_to_next_stage()
			throws Throwable {
		for (int i = 0; i <= 200; i++) {
			try {
				toastMessage = javascriptHelper
		...
		softAssert.assertTrue(toastMessage.contains("L1APP"),
				" Record is not submitted to the next stage hence failed");

	}

	@And("^user_076 extract the user ID for consumer durables which submitted on data entry stage$")
	public void user_076_extract_the_user_id_for_consumer_durables_which_submitted_on_data_entry_stage()
			throws Throwable {
		System.out.println("Toast message " + toastMessage);
		String[] split = toastMessage.split("\\s");
		for (String string : split) {
			System.out.println(string);
		}
		System.out.println(split.length);
		String trimUserID = toastMessage.substring(82);
		System.out.println("Trimmed User ID " + trimUserID);
		String finalUserID = trimUserID.replace("."," ").trim();
		System.out.println("Final User ID " + finalUserID);
		credntialsExcelTestData.updateTestData("userType_076_03", "UserName", finalUserID);
	}

	@And("^user_076 invoke soft assert in application details in app data entry stage$")
	public void user_076_invoke_soft_assert_in_application_details_in_app_data_entry_stage() throws Throwable {
		softAssert.assertAll();
	}
