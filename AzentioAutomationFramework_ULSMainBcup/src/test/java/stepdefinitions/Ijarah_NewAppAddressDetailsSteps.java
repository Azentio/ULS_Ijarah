...
public class Ijarah_NewAppAddressDetailsSteps extends BaseClass {
	...
	ExcelData excelTestDataForAddressDetails = new ExcelData(jrahaTestDataPath, "NewApp_AddressDetailsTestData",
			"Dataset ID");
	BrowserHelper browserHelper = new BrowserHelper(driver);

	ExcelData murabahExecutionSheet = new ExcelData(murabahaTestDataPath, "Murabaha_ExecutionTracker", "TestCase ID");

	ExcelData ijarahExecutionSheet = new ExcelData(jrahaTestDataPath, "Ijarah_ExecutionTracker", "TestCase ID");
	...
	ExcelData excelTestDataForMurabahaAddressDetails = new ExcelData(murabahaTestDataPath,
			"M_NewApp_AddressDetailsTestData", "Dataset ID");

	// Tawruqq
	ExcelData excelTestDataForTawruqqAddressDetails = new ExcelData(tawruqqTestDataPath,
			"TW_NewApp_AddDetails_TestData", "Dataset ID");
	...
	public void get_the_test_data_for_test_case_id_at_ij_new_app_ad_03() throws Throwable {
		addressDetailsExecutionData = ijarahExecutionSheet.getTestdata("AT_IJ_NewApp_AD_03");
		addressDetailsTestData = excelTestDataForAddressDetails
				.getTestdata(addressDetailsExecutionData.get("dataSet_ID"));
	}
		...
		for (int i = 0; i <= 300; i++) {
			try {
				recordStatus = javascriptHelper
						.executeScript("return " + newAppAddressDetailsJSElements.getElement("addressdetails_status"))
						.toString();
				if (!(tempRecordStatus.equals(recordStatus))) {
					System.out.println("Temp Record status " + tempRecordStatus);
					System.out.println("Original Record Status " + recordStatus);
					break;
				}
			} catch (Exception e) {
				if (i == 300) {
