...
//                    if (moduleName.equalsIgnoreCase(testData.get("Module Name"))) {
	...
	public void user_click_the_mail_box_in_uls_application() throws Throwable {
		System.out.println("Mail JS "+commonJSElements.getElement("mailBox"));
		for (int i = 0; i <= 100; i++) {
			try {
	...
	public void user_click_the_entitle_button_under_inbox() throws Throwable {
		...
		for (int i = 0; i <= 1000; i++) {
			try {
				javascriptHelper.executeScriptWithWebElement(commonJSElements.getElement("inboxEntitleBtn")).click();
				break;
			} catch (Exception e) {
				if (i == 1000) {
					Assert.fail(e.getMessage());
...
//	AT_INCD_05
	...
	public void user_verify_self_employed_customer_available_under_income_section_in_customer_financials_tab()
			throws Throwable {
				...
				try {
					...
					if (recordName.length() > 0) {
						if ((recordName.trim()).equalsIgnoreCase(("Self Employed").trim())) {
							...
							softAssert.assertTrue(text.contains("Self Employed"),
									"Self Employed value should be displayed");
							break first;
						} else {
							continue;
						}
