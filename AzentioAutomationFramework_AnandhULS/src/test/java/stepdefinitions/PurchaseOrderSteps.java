...
public class PurchaseOrderSteps extends BaseClass {
	...
	JSPaths purchaseOrderJSpaths = new JSPaths(jsFilePath, "PurchaseOrder_JSPaths", "PurchaseOrder_FieldName",
			"JSPaths");
	JSPaths jsPaths = new JSPaths(jsFilePath, "Ijara_loginElements", "Ijara_LoginFieldName", "JSPath");
		...
		for (int j = 0; j < premitiveNumberOfRecordInMailBox; j++) {
			for (int k = 0; k <= 150; k++) {
				try {
					...
					if (recordRefNumber
							.equals(configFileReader.getMurabahaPurchaseOrderApprovalRecordReferenceNumber())) {
