...
public class JavascriptHelper {
	...
	public WebElement executeScriptWithWebElement(String script) {
		clicksAndActionsHelper = new ClicksAndActionsHelper(driver);
		JavascriptExecutor exe = (JavascriptExecutor) driver;

		WebElement element = (WebElement) exe.executeScript("return " + script);
		clicksAndActionsHelper.scrollIntoView(element);
		return (WebElement) exe.executeScript("return " + script);
	}
...
//scrollDownVertically
	public void scrollDownVertically() {
		executeScript("window.scrollTo(-10000, document.body.scrollHeight)");
		// ExtentTestManager.getTest().info("scrollDownVertically");
	}

//scrollUpVertically
	public void scrollUpVertically() {
		executeScript("window.scrollTo(99999, -document.body.scrollHeight)");
