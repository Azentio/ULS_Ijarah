...
public class Murabha_NewAppAddressDetailsSteps extends BaseClass {
	...
	ExcelData murabahExecutionSheet = new ExcelData(excelTestDataPath, "Murabha_ExecutionTracker", "TestCase ID");

	ExcelData ijarahExecutionSheet = new ExcelData(excelTestDataPath, "Ijarah_ExecutionTracker", "TestCase ID");
	JSPaths commonJSElements = new JSPaths(jsExcelPath, "Murabha_CommonElements", "Murabha_CommonFieldName", "JSPath");
	...
	public void user_076_click_on_add_button_in_address_details_at_new_app_stage() throws Throwable {
		...
		for (int i = 0; i <500; i++) {
			try {
				...
				if (!length.isBlank()&&!length.equalsIgnoreCase("1")&&!length.equalsIgnoreCase("1")) {
					break;
				}
			} catch (Exception e) {
				if (i==499) {
