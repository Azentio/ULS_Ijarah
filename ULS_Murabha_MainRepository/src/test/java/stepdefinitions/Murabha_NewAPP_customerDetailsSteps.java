...
public class Murabha_NewAPP_customerDetailsSteps extends BaseClass {
	WebDriver driver = BaseClass.driver;
	ConfigFileReader configFileReader = new ConfigFileReader();

	String jsPathExcelfile = configFileReader.getJSFilePath();
	String excelTestData = configFileReader.getMurabhaTestDataFilePath();
	...
	JSPaths CommonJsElements = new JSPaths(jsPathExcelfile, "Murabha_CommonElements", "Murabha_CommonFieldName",
			"JSPath");
	Robot robot;
	...
	public void user_076_search_for_murabaha_record_reference_number_in_new_app_stage() throws Throwable {
		System.out.println(CommonJsElements.getElement("mail_box_search_text"));
		for (int i = 0; i <= 150; i++) {
			try {
				javascriptHelper
						.executeScriptWithWebElement(CommonJsElements.getElement("mail_box_search_text")).sendKeys(configFileReader.getMurabahaRecordRefereneNumber());
				break;
			} catch (Exception e) {
				if (i == 150) {
					Assert.fail(e.getMessage());
